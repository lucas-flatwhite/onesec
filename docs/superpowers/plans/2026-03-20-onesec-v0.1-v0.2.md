...
dependencies = [
    ...
    "rich>=13",
    "pydantic>=2",
    # tomllib is stdlib in Python 3.11+; this project requires 3.12 — no tomli needed
    "ffmpeg-python>=0.2",
...
def load_config(path: str | Path) -> Config:
    ...
    TOML structure:
      [output]   → clip_duration, transition, ...
      [device]   → device key (e.g. device = "cpu")
      [parallelism] → workers
      [analyzers] → name = { enabled, weight, options }
    Note: [device] section uses `device` as the key name (matching Config.device field).
    """
    path = Path(path)
    if not path.exists():
    ...
    device_section = data.get("device", {})   # [device] section → dict
...
def test_cli_run_dry_run_json(synthetic_video, tmp_path):
    ...
    if output.startswith("["):
        # Segments found — validate JSON structure
        data = json.loads(output)
        assert isinstance(data, list)
        if data:
            assert "video_path" in data[0]
    # else: "No segments found" message — also valid
...
- [ ] **Step 5: Update `__init__.py` — add audio & motion imports and update _build_default_analyzers**

In `src/onesec/__init__.py`, make these two changes:

**Change 1** — add imports after the SceneAnalyzer import line:
```python
# Old:
from onesec.analyzer.scene import SceneAnalyzer
...
**Change 2** — replace `DEFAULT_ANALYZERS` and `_build_default_analyzers`:
```python
# Old:
DEFAULT_ANALYZERS = ["scene"]  # v0.2 will add audio, motion
...
- [ ] **Step 5b: Update `cli.py` — add audio & motion to _BUILTIN_ANALYZERS and analyzers_list**

In `src/onesec/cli.py`, make these two changes:

**Change 1** — imports (add after SceneAnalyzer import):
```python
from onesec.analyzer.audio import AudioAnalyzer
from onesec.analyzer.motion import MotionAnalyzer
```

**Change 2** — replace `_BUILTIN_ANALYZERS`:
```python
# Old:
_BUILTIN_ANALYZERS: dict[str, type[Analyzer]] = {
    "scene": SceneAnalyzer,
...
**Change 3** — in `analyzers_list()`, add rows:
```python
table.add_row("scene",  "1", "No", "✓", "Histogram-based scene change")
